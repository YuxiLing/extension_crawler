...
def handle_missed_file(missed_id, missed_dir, recent_dir):
    for item_id in missed_id:
        ...
        try:
            ...
            print("handle_missed_file: success move:",item_id,file=open('opera_log.txt','a'))
        except:
            print("handle_missed_file: no file ",item_id,file=open('opera_log.txt','a'))
...
def download_new_add_ext(new_add_item, current_dir):
    ...
    for item in new_add_item:
        ...
        try:
            ...
            print('download failed:',id, file=open("./opera_log.txt", "a"))
    print("download successful total number:",suc, file=open("./opera_log.txt","a"))
    print('Empty file total number: ',num, file=open("./opera_log.txt", "a"))


def _DownloadCrxFromCws(ext_key, link,dst):
    """Downloads CRX specified from Chrome Web Store.
    Retrieves CRX (Chrome extension file) specified by ext_id from opera
    Store, into directory specified by dst.
    Args:
...
def missed_all_app(new_file, count):
    ...
    if(count != 0):
        num=count-1
        last_file = './malicious_ext_crawler/data/full_list/opera_ext_data_[%s]FILTER_KEYWORDS.json' % num
...
if __name__ == "__main__":
    missed_all_app('./malicious_ext_crawler/data/full_list/opera_ext_data_[1]FILTER_KEYWORDS.json',0)